...
def test_session_manager():
    def run_operations_for_session(session, session_id, has_server=True):
        ...
        try:
            ...
            session.add_topic(id=thread_id + 1, name=all_topics[thread_id])
            session.remove_topic(thread_id + 1)
            session.add_topic(id=thread_id + 1, name=all_topics[thread_id])
            logging.info(f"Session:{session_id} Topic Handling Passed!")

            session.add_server(chosen_inviter_id) if has_server else None
            session.add_server_role_id(chosen_role_id)
            logging.info(f"Session:{session_id} Adding Roles Passed!")
            ...
            if has_server:
                ...
                topics = session.return_topics_for_server()
                logging.info(f"Session:{session_id} Topics for server: {topics}")

                session.remove_topic_for_server(choice(topics))
                remaining = session.return_topics_for_server()
                if len(remaining) == len(topics) - 1:
                    logging.info(f'Session:{session_id} Topic deletion success')
                else:
                    raise Exception("Couldn't delete topics")

                session.remove_server()
                logging.info(f'Session:{session_id} Server removal Passed!')

        except Exception as e:
            logging.error(f"Session {session_id} operation error: {e}")
        finally:
            session.close_pool()
